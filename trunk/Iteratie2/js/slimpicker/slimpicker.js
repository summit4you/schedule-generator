...
var SlimPicker = new Class({
	...
	setCurrentDate: function() {
		...
		this.now = new Date();
		if (inputValue != '') {
			this.current=this.getDateFromFormat(inputValue,this.options.format)
...
//					this.moveFocus(e.shift);
	...
	getDateFromFormat: function(val,format) {
		...
		var startY=format.indexOf('yyyy');
		var startM=format.indexOf('MM');
		var startD=format.indexOf('dd');
		var month=parseInt(val.substring(startM,startM+2))-1;
		return new Date(val.substring(startY,startY+4),month,val.substring(startD,startD+2));
	}
