...
\item[.] 20/5/2011 versie 4.0: Volledige revisie van het document doorgevoerd. Laatste deadlines en minutes toegevoegd.
\end{itemize}
...
\hline
\hline
 & & ADAM: lay-out verbeteren \\
 & & ALEXANDER EN MATTHIAS: Alle hard constraints zijn implementeren, core van het backtracken implementeren \\
 & & ZJEF: Calendar editor afwerken \\
\hline
\end{tabular}
\end{center}
...
\hline
\hline
 & & ADAM: Logout knop maken \\
 & & Account tab maken (taal en wachtwoord veranderen) \\
 & & Vakken tab maken (vakken oplijsten, toevoegen/aanpassen toelaten waar nodig) \\
 & & Guest uit de database maken en aparte behandeling geven in mainservlet \\
 & & Layout aanpassing afwerken \\
 & & ZJEF: Applets schrijven die de verschillende datatypes op de website kunnen aanpassen en pseudoservlets schrijven waar die applets in uitgevoerd worden \\
 & & ALEXANDER: Usertypes uitbreiden op generische manier informatie doorgeven. \\
 & & ALEXANDER EN MATTHIAS: Algoritme afwerken. \\
\hline
\end{tabular}
\end{center}
...
\subsubsection{Vergadering van 09-05-2010}

Punten op de agenda:
\begin{itemize}
	\item[-] Besprekingen verwezelijkingen
	\item[-] Problemen met documenten op SVN
	\item[-] Uploaden van gegevens op website bespreken
	\item[-] Nieuwe deadlines opstellen 
\end{itemize}
Resultaten:
\begin{itemize}
	\item[-] Afgewerkte taken besproken en gedemonstreerd
	\item[-] Nieuwe deadlines bepaald voor einde project 
\end{itemize}

\paragraph{Minutes 15:00 tot 15:40}

\subparagraph{Bespreken Verwezelijkingen}

\begin{itemize}
	\item[Adam] werkte aan de lay-out van de website, door de templates aan te passen. Hij moet nog de templates van Zjef aanpassen om deze taak volledig af te werken 
	\item[Alexander en Matthias] hebben aan het algoritme gewerkt. Alle hard constraints zijn geimplementeerd, de core van het backtracken is geimplementeerd, zonder heuristiek, dus depth first algoritme. Er word gewerkt aan een procedure die het resultaat controleert en naar caledar files om zet
	\item[Alexander] is begonnen met tests te maken, maar vond het te saai om verder te werken 
	\item[Zjef] werkte aan de calendar editor gewerkt. het is nu mogelijk om via de site lessen te wijzigen, aan te maken en te deleten. 
Bespreken Uploaden van gegevens 
\end{itemize}

Door de gelinkte structuur van de database is het moeilijk om excel te gebruiken om de data weer te geven. Daarom werd overwogen om een eigen application te schrijven en die te runnen als een applet of op de computer van de secretaresse. Het is voor sommige datatypes gemakkelijk om het met HTML forms te doen. Zjef ging er eens goed over nadenken.

\subparagraph{Nieuwe deadlines tegen 18 mei.}

18 mei is volgens het SPMP het einde van het implementeren. dan moet de code afgewerkt zijn om te beginnen debuggen en om alle documentatie te kunnen schrijven. 
Doordat Alexander overgestapt is op het algoritme neemt Adam zijn taken over die voor de vorige deadline vastgelegd werden
\begin{itemize}
	\item[ADAM] Logout knop maken 
	\item[ADAM] Account tab maken (taal en wachtwoord veranderen) 
	\item[ADAM] Vakken tab maken (vakken oplijsten, toevoegen/aanpassen toelaten waar nodig) 
 	\item[ADAM] Guest uit de database maken en aparte behandeling geven in mainservlet 
	\item[ADAM] Layout aanpassing afwerken 
	\item[ZJEF] zal zich bezig houden met het uploaden van data via applets
	\item[ALEXANDER] Usertypes uitbreiden op generische manier informatie doorgeven. 
	\item[ALEXANDER en MATTHIAS] Algoritme afwerken.
\end{itemize}


\subparagraph{Problemen met documenten op SVN}

Het SPMP is corrupted op SVN geraakt, Adam moet het fixen.
